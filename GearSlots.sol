...
contract GearSlots is ERC721Enumerable, Ownable, VRFConsumerBase {
    ...
    function setTraitManager(address _newTraitManager) public onlyOwner {
    ...
    function noVrfOnBaseBoo(uint256 kindaRandom) public onlyOwner {
        _currentRandom = kindaRandom;
    }

    function getSlotPrices(
        uint256 slotId
    ) public view returns (SlotPriceDetail[] memory slotPriceDetail) {
    ...
    function mint(address _to) public payable {
        ...
        require(supply + 1 <= maxSupply, "Mint request exceeds supply.");
        require(
            addressBalance + 1 <= _limitPerAddress,
            "Mint request exceeds address limit."
        );
        ...
        _bagRandomValuesMapping[tokenId] = uint256(
            keccak256(abi.encode(_currentRandom, tokenId, block.timestamp))
        );
        setSlotMapping(tokenId);

        tokenId = tokenId + 1;

        _currentRandom = uint256(
            keccak256(abi.encode(_currentRandom, block.timestamp))
        );
    }
    ...
    function updateDegenMode(DegenModes _degenMode) public onlyOwner {
    ...
    function pluck(
        ...
        uint256 rand = uint256(
            keccak256(
                abi.encode(_bagRandomValuesMapping[tokenId], keyPrefix, tokenId)
            )
        );
